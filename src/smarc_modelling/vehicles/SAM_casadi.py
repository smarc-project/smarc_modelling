...
#                            self.damping_rot, self.damping_rot, self.damping_rot)
    ...
    def calculate_propeller_force(self, u):
        ...
        for i in range(n_rpm.size1()):
            ...
            if use_Va:
                Jb = self.J_eff(Va_abs, D, n, self.Ja_max, n0_rps=n0_rps)
                KT_fwd = self.KT_0 + gn * (self.KT_max - self.KT_0)/self.Ja_max * Jb
                KQ_fwd = self.KQ_0 + gn * (self.KQ_max - self.KQ_0)/self.Ja_max * Jb
            else:
                KT_fwd, KQ_fwd = self.KT_0, self.KQ_0  # no Va dependence

            # NOTE: Same coefficients for forward and backwards right now
            cT = rho * (D**4) * KT_fwd
            cQ = rho * (D**5) * KQ_fwd
    
            X_fwd = cT * n_rps[i] * nabs # thrust ~ n|n|
            K_fwd = cQ * n_rps[i] * nabs # torque ~ n|n|
            X_rev = cT * n_rps[i] * nabs # thrust ~ n|n|
            K_rev = cQ * n_rps[i] * nabs # torque ~ n|n|
    ...
    def J_eff(self, Va_abs, D, n_rps, Ja_max, n0_rps=3.0, Jsat=None):
        if Jsat is None:
            Jsat = Ja_max/3.0
        a = ca.sqrt(n_rps*n_rps + (n0_rps*n0_rps))   # constant floor in rps
        J_raw = Va_abs / (D * a)
        return Ja_max * (1 - ca.exp(-J_raw/Jsat))    # ∈ [0, Ja_max], C^∞
    ...
    def J_safe(self, Va, D, n_abs, Ja_max, Jsat=None, eps=1e-6):
        ...
        Jb = Ja_max * (1 - ca.exp(-J/Jsat))   # C∞, monotone, <= Ja_max
        return Jb
