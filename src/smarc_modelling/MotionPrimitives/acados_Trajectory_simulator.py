...
from smarc_modelling.MotionPrimitives.control import *
...
def MPC_optimization(waypoints, map_instance):
    ...
    Ts = glbv.RESOLUTION_DT         # Sampling time
    N_horizon = len(waypoints)      # Prediction horizon
    nmpc = NMPC_trajectory(model, Ts, N_horizon)

    trajectory = np.asarray(waypoints, dtype='float32')

    Nsim = 1#(trajectory.shape[0]-1)
    ...
    for i in range(Nsim):
        ...
        for stage in range(N_horizon):
            ocp_solver.set(stage, "p", trajectory[stage,:])

        references = np.vstack([references, trajectory[0,:nx]])
        ocp_solver.set(N_horizon, "yref", trajectory[-1,:nx])
