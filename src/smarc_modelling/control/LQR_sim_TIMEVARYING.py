...
def save_csv(trajectory, t, i, case):
    ...
    file_path (str): The path to the CSV file.
    """
    # np.savetxt("/home/admin/smarc_modelling/src/Trajectories/report_update/"+ case + "/LQR_new/" + case + str(i) +".csv"
    #           , trajectory, delimiter=',', header="X,Y,Z,phi,theta,psi,vx,vy,vz,p,q,r,control", comments='')
    # np.savetxt("/home/admin/smarc_modelling/src/Trajectories/report_update/"+ case + "/LQR_new/time_" + case + str(i) +".csv"
    #           , t, delimiter=',', header="time (ms)", comments='')
    np.savetxt("/home/admin/smarc_modelling/src/Trajectories/report_update/noise/lqr_new/rotation/" + str(case) +"_" + str(i) +".csv"
               , trajectory, delimiter=',', header="X,Y,Z,phi,theta,psi,vx,vy,vz,p,q,r,control", comments='')
    ...
    for std in [0.0]:
        for j in range(0,100):
            ...
            if i == Nsim - 1:
                ...
                print(f"Simulation stopped at step {i} due to an error.")
            # evaluate timings
            ...
            #save_csv(sim, t, j, bias_set)   #Ändra till bias set för olika noise
            part_plot_function(trajectory, sim[:i-1], u_dot[:i-1])

            error = np.linalg.norm(simNonlinear[-1, :3]-trajectory[-1, :3])
            if error <= 0.5:
                success +=1
        
        print(f"Number of successes: {success/100}")
